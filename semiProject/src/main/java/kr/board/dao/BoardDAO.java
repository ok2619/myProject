		...
		try {
			...
			if (keyword != null && !"".equals(keyword)) {
				if (keyfield.equals("1"))
					...
					sub_sql = "WHERE b.b_content LIKE ?"; // 이름검색은 일단 안하는걸로 할게요! (필요하면 추후에 추가)
			}
...
//ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ	
	...
	public BoardVO getBoard(int board_num) throws Exception {
		...
		try {
			...
			conn = DBUtil.getConnection();
			// SQL문 작성
			sql = "SELECT * FROM qboard b JOIN qmember m " + "ON b.user_num=m.user_num WHERE b.board_num=?";
			// preparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
...
//ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
	...
	public void updateBoard(BoardVO board) throws Exception {
		...
		try {
			...
			// SQL문 작성
			sql = "UPDATE qboard SET title=?,b_content=?,modify_date=SYSDATE" + sub_sql + ",ip=? WHERE board_num=?";
			// preparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
...
//ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
	...
	public List<BoardReplyVO> getListReplyBoard(int startRow, int endRow, int board_num) throws Exception {
		...
		try {
			...
			sql = "SELECT * FROM (SELECT a.*, rownum rnum FROM "
					...
					+ "JOIN qmember m USING(user_num) WHERE b.board_num=? " + "ORDER BY b.re_num DESC)a) "
					+ "WHERE rnum >= ? AND rnum <= ?";
	...
	public void updateReplyBoard(BoardReplyVO reply) throws Exception {
		...
		try {
			...
			conn = DBUtil.getConnection();
			// SQL문 작성
			sql = "UPDATE qboard_reply SET re_content=?, re_modifydate=SYSDATE, re_ip=? WHERE re_num=?";
			// PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터를 바인딩
	...
	public void deleteReplyBoard(int re_num) throws Exception {
		...
		try {
			...
			conn = DBUtil.getConnection();
			// SQL문 작성
			sql = "DELETE FROM qboard_reply WHERE re_num=?";
			// PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
			pstmt.setInt(1, re_num);
	...
	public List<BoardVO> getMyReview(int startRow, int endRow, String keyfield, String keyword, String id)
			throws Exception {
		...
		try {
			...
			// SQL문 작성
			sql = "SELECT * FROM (SELECT a.*, rownum rnum FROM "
					+ "(SELECT * FROM qboard b JOIN qmember m USING(user_num) where m.id=?" + sub_sql
					+ "ORDER BY b.board_num DESC)a) " + "WHERE rnum >=? AND rnum <= ?";
...
/////////////////////////////////좋아요/////////////////////////////////
	...
	// 좋아요 누른 회원 저장
	public void insertFav(int board_num, int user_num) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
		...
		try {
			conn = DBUtil.getConnection();
			sql = "INSERT INTO qboard_fav (fav_num,board_num,user_num) " + "VALUES (qfav_seq.nextval,?,?)";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, board_num);
	...
	// 회원이 동일한 게시물에 좋아요를 눌렀는지 확인
	public BoardFavVO selectFav(int board_num, int user_num) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		try {
			conn = DBUtil.getConnection();
			sql = "SELECT * from qboard_fav where board_num=? and user_num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, board_num);
			pstmt.setInt(2, user_num);
			rs = pstmt.executeQuery();
			...
			if (rs.next()) {
				...
				fav.setFav_date(rs.getDate("fav_date"));
			}
	...
	// 좋아요 개수
	public int selectFavCount(Integer board_num) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		try {
			conn = DBUtil.getConnection();
			sql = "SELECT COUNT(*) from qboard_fav where board_num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, board_num);
			rs = pstmt.executeQuery();
	...
	// 좋아요 취소
	public void deleteFav(Integer fav_num) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
		...
		try {
			conn = DBUtil.getConnection();
			sql = "DELETE FROM qboard_fav WHERE fav_num=?";
			pstmt = conn.prepareStatement(sql);
	...
	// 게시물 삭제시
	public void deleteFavByBoardNum(Integer board_num) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
		...
		try {
			conn = DBUtil.getConnection();
			sql = "DELETE FROM qboard_fav WHERE board_num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, board_num);
			pstmt.executeUpdate();
